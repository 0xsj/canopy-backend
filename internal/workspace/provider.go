...
func Wire(db *database.DB, orgMembers service.OrgMemberReader, pub events.Publisher, log logger.Logger) *Provider {
	dbtx := db.DBTX()
	...
	svc := service.New(
		...
		func(tx database.DBTX) domain.WorkspaceMemberRepository { return postgres.NewWorkspaceMemberRepository(tx) },
		pub, log,
	)
